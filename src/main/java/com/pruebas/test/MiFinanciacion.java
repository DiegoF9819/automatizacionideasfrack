...
public class MiFinanciacion extends Base {
    ...
    private By financiacionLink = By.xpath("//p[contains(text(), 'Mi Financiación')]"); // Selector para el enlace "Mi Financiación"

    public MiFinanciacion(WebDriver driver) {
        super(driver);
    }
    ...
    public void ejecutarLista() {
        ...
        // Hacer clic en el enlace "Mi Financiación"
        clickEnFinanciacion();
    }
    ...
    private void clickEnFinanciacion() {
        WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(30));
        try {
            WebElement financiacionElement = wait.until(ExpectedConditions.elementToBeClickable(financiacionLink));
            if (financiacionElement.isDisplayed() && financiacionElement.isEnabled()) {
                financiacionElement.click();
            } else {
                System.out.println("El enlace 'Mi Financiación' no está visible o habilitado.");
            }
        } catch (Exception e) {
            System.out.println("No se pudo hacer clic en el enlace 'Mi Financiación': " + e.getMessage());
        }
    }
}
