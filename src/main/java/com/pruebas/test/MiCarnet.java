...
public class MiCarnet extends Base {
    ...
    private By miCarnetLink = By.cssSelector("a.nav-link[href='micarnet.php']"); // Enlace "Mi carnet"

    public MiCarnet(WebDriver driver) {
        super(driver);
    }
    ...
    public void ejecutarListaCotizaciones() {
        ...
        click(this.rol);
        selectOption(2, this.rol); // Selecciona la opción "Estudiante", que es la opción 3 (índice 2)
    ...
    private void navigateToMiCarnet() {
        WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(10));
        try {
            WebElement miCarnetElement = wait.until(ExpectedConditions.elementToBeClickable(miCarnetLink));
            if (miCarnetElement.isDisplayed() && miCarnetElement.isEnabled()) {
                miCarnetElement.click();
            } else {
                System.out.println("El enlace 'Mi carnet' no está visible o habilitado.");
            }
        } catch (Exception e) {
            System.out.println("No se pudo hacer clic en el enlace 'Mi carnet': " + e.getMessage());
        }
    }
